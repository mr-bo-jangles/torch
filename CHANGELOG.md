...
- [FEATURE] (@mr-bo-jangles) Light sources can now have a `duration` (in minutes of world time). When lit, the module tracks elapsed world time and automatically extinguishes the source when it runs out.
  ...
  - User-configurable via the existing JSON/YAML light source file — set `duration` on any source, or `0` to disable.
- [FEATURE] (@mr-bo-jangles) Low-fuel warnings whispered to the token owner before a light source expires. Controlled by a new **Duration Warning Threshold** setting (default 0.9 — warns at 90% elapsed, 10% remaining). Set to 0 to disable warnings.
- [FEATURE] (@mr-bo-jangles) New `torch.expired` hook fires when a source burns out, for macro authors.
- [FEATURE] (@mr-bo-jangles) New `game.Torch.remainingDuration(sceneId, tokenId)` API method.
- [FEATURE] (@mr-bo-jangles) Debug logging now behind a **Debug Logging** game setting (off by default).
...
- [BUGFIX] (StormDarkwood) Fixed Dragonbane lighting _back_ to what it should be. (It's in meters, not grids.)
