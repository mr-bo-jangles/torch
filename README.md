...
This module sheds light from the location of a player token upon demand based upon equipment inventory. It is recommended _not_ to use this module for spells or equipment that have other capabilities you intend to use, like performing damage or setting down the equipment, but to rely upon other common approaches, like active effects or item piles, for those.
...
Also, light source identification is by name and the built-in light sources are in English. This means you may need to create a custom light source file to supply aliases in the language in which you're playing the game. Of course, if I were provided the localized names, localized aliases _could_ find their way into the built-in list over time as well, streamlining play.
...
- Light sources can have a `duration` (in minutes of world time). When a source with a duration is lit, the module tracks elapsed world time and automatically extinguishes the source when its duration runs out, consuming inventory if applicable. A configurable **Duration Warning Threshold** (default 0.9) controls when a low-fuel warning is whispered to the token's owner — at 0.9, the warning fires when 90% of the duration has elapsed (10% remaining). Set the threshold to 0 to disable warnings. Duration tracking requires the GM to advance world time (manually or via a module like Simple Calendar). Built-in durations are provided for D&D 5e, Pathfinder 1e, and Pathfinder 2e equipment sources.

It all depends how you want to run your game.

The bright and dim light settings are intended only as a fallback for use when you are playing in a system where nobody has supplied a set of light sources. The settings will appear with the light source name specified with them. (In systems which _do_ have a set of light sources, the fallback name identifies the source to use if the user hasn't yet selected a light source.)
  ...
  "dnd5e": {
    ...
    "sources": {
      "Candle": {
        ...
        "consumable": true,
        "states": 2,
        "duration": 60, // Optional - minutes of world time the source lasts (0 or omitted = infinite)
        "light": [
          // If you have only 2 states, you can supply a single set of light stats rather than an array
          {
            "bright": 10,
            "dim": 15,
            "angle": 360,
...
- The `sources` property has a sub-property for each light source you are creating or superseding. For each source:
  ...
  - The `duration` value specifies how long the light source lasts, in minutes of world time. When a source with a duration is lit, the module tracks elapsed world time and automatically extinguishes it when the duration runs out. A low-fuel warning is whispered to the token's owner before expiry, controlled by the "Duration Warning Threshold" game setting. If omitted or set to `0`, the source burns indefinitely.
...
### `game.Torch.remainingDuration(sceneId, tokenId)`

Returns the number of minutes remaining on the current light source's duration for the specified token. Returns `null` if the source has no duration (infinite) or no duration tracking is active.
...
### `"torch.expired" token, source`

A light source's duration ran out and it was automatically extinguished. This fires just before the corresponding `"torch.changed"` hook with state `"off"`.

- **token**: The token whose light source expired
- **source**: The name of the light source that expired
